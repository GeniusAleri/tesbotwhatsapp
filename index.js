const crypto = require("crypto"); // tambahin manual
const {
...
async function startBot() {
  ...
  sock.ev.on("connection.update", async (update) => {
    ...
    if (connection === "close") {
      ...
      if (reason === 401) {
        ...
        io.emit("status", "Koneksi terputus, mencoba menyambung ulang...");
        startBot();
      }
  ...
  sock.ev.on("messages.upsert", async (m) => {
    ...
    const [rows] = await db
      .promise()
      .query("SELECT keyword, reply_message, type_message FROM auto_replies");
    ...
    for (const row of rows) {
      ...
      if (regex.test(text)) {
        ...
        await db
          .promise()
          .query("INSERT INTO all_messages (sender, message) VALUES (?, ?)", [
            senderNumber,
            text,
          ]);
        ...
        if (keyword === "/stop") {
          if (activeSessions[senderJid]) {
            ...
            const randomStopResponse =
              stopResponses[Math.floor(Math.random() * stopResponses.length)];
